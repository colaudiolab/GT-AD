...
class PatchEmbeding(nn.Module):
    ...
    2D Image to Patch Embedding
    """
    def __init__(self, args, norm_layer=None):
        """
        Map input tensor to patch.
        Args:
            image_size: input image size
...
class VITST(nn.Module):
    def __init__(self, args):
        ...
        freeze = True
        self.vision_transformer = vit_base_patch16_224_in21k()# B/16
        ...
        self.patch_embeding = PatchEmbeding(args)
        self.linear = nn.Linear(self.patch_num * self.embed_dim, 1080)
    ...
    def forward(self, x):
        ...
        x = self.linear(x.reshape(B*M,-1)) #嵌入层特征
        return x
